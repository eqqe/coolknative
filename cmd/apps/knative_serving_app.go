...
func MakeInstallKnativeServing() *cobra.Command {
	...
	knativeServing.RunE = func(command *cobra.Command, args []string) error {
		...
		res, err := kubectlTask("apply", "-f",
			"https://github.com/knative/serving/releases/download/v0.17.3/serving-crds.yaml")
		if err != nil {
			return err
		}
		if res.ExitCode != 0 {
		...
		res, err = kubectlTask("apply", "-f",
			"https://github.com/knative/serving/releases/download/v0.17.3/serving-core.yaml")
		if err != nil {
			return err
		}
		if res.ExitCode != 0 {
		...
		res, err = kubectlTask("apply", "-f",
			"https://github.com/knative/serving/releases/download/v0.17.3/serving-hpa.yaml")
		if err != nil {
			return err
		}
		if res.ExitCode != 0 {
		...
		res, err = kubectlTask("apply", "-f",
			"https://github.com/knative-sandbox/net-kourier/releases/download/v0.17.2/kourier.yaml")
		if err != nil {
			return err
		}
		if res.ExitCode != 0 {
		...
		if publicIp != "localhost" {
			...
			cmd = exec.Command("kubectl", "-n", "knative-serving", "set", "env", "deployment/3scale-kourier-control", "CERTS_SECRET_NAMESPACE=knative-serving")
			output, err = cmd.CombinedOutput()
			...
			cmd = exec.Command("kubectl", "-n", "knative-serving", "set", "env", "deployment/3scale-kourier-control", "CERTS_SECRET_NAME=tls")
			output, err = cmd.CombinedOutput()
