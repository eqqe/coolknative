...
metadata:
  name: {{.Namespace}}
  labels:
    knative-eventing-injection: "{{.KnativeEventingInjection}}"
---
apiVersion: v1
data:
    ...
    - name: MINIO_SECRET_KEY
      valueFrom:
        secretKeyRef:
          name: minio
          key: secretkey
    - name: MINIO_BUCKET_LOGS
      value: logs
    - name: MINIO_BUCKET_LOGS_EXPIRATION_DAYS
      value: "90"
    image: eqqe/minio_lifecycle:0.1.0
---
apiVersion: tekton.dev/v1beta1
kind: Task
metadata:
...
spec:
  ...
  - name: install-infra-step-coolknative-knative-eventing
    ...
    - install
    - knative-eventing
  - name: install-infra-step-coolknative-fluent
    args:
    - install
    - fluent
    - --namespace
...
spec:
  ...
  - name: copy-file-resources
    ...
    - -c
    - |
      mc config host add minio http://minio-hl-svc.minio:9000 $MINIO_ACCESS_KEY $MINIO_SECRET_KEY --api S3v4
...
spec:
  ...
  steps:
  - name: run-tests
    image: eqqe/python-docker-utils-webservices:3.7-slim
    env:
    - name: PYTHONPATH
      value: ..
  ...
  - name: copy-file-resources
    ...
    resources:
      ...
      - name: workspace
        resource: file-resources-git
  - name: unit-tests
    runAfter: [copy-file-resources, minio-lifecycle-task]
    taskRef:
      name: unit-tests
    resources:
      inputs:
